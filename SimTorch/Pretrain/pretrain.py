...
def main(argv):
    ...
    #optimizer = optim.SGD(list(encoder.parameters())+list(proj_head.parameters()),lr=FLAGS.lr,weight_decay=FLAGS.weight_decay,momentum=FLAGS.momentum)
    optimizer = LARS(base_optimizer, trust_coef=0.001)
    #optimizer = apex.optimizers.FusedLAMB(list(encoder.parameters()) + list(proj_head.parameters()),lr=FLAGS.lr, weight_decay=FLAGS.weight_decay )
    #optimizer = optim.Adam(list(encoder.parameters()) + list(proj_head.parameters()), lr=FLAGS.lr, weight_decay=FLAGS.weight_decay, amsgrad=True)
