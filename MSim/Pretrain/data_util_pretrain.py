...
import TensorFlowTransf as flow
...
def random_brightness(image, max_delta):
    #Multiplicative change of brightness.
...
def random_darker(image, p):
    darker_t = functools.partial(flow.darker, number=tf.random.uniform([], 0, 0.19))
    return random_apply(darker_t, p=p, x=image)

def random_sharp(image, p):
    sharp_t = functools.partial(flow.sharpness, magnitude=tf.random.uniform([], 0.2, 0.999))
    return random_apply(sharp_t, p=p, x=image)

def random_solarize_plus_invert(image, p):
    return random_apply(flow.solarize_plus_invert, p=p, x=image)

def random_sobel(image, p):
    return random_apply(flow.sobel_edges, p=p, x=image)

def random_sharpness_with_posterize(image, p):
    return random_apply(flow.functions_rand, p=p, x=image)
