...
def main(argv):
    ...
    sup_head = md.Supervised_Head(encoder.representation_dim, FLAGS.number_of_classes).cuda()

    #Build optimizer and supervised loss
    base_optimizer = optim.SGD(list(sup_head.parameters()),lr=FLAGS.lr,weight_decay=FLAGS.weight_decay,momentum=FLAGS.momentum)
    ...
    def save_model(encoder, sup_head, epoch_number, optimizer, scheduler):
        torch.save({
    ...
    if not FLAGS.start_from_scratch:
        ...
        if os.path.isfile(FLAGS.checkpoint_dir + '/model.pth'):
            checkpoint = torch.load(FLAGS.checkpoint_dir + '/model.pth')
    ...
    for epoch in range(start_epoch, start_epoch + FLAGS.epochs):
        ...
        with torch.no_grad():
            for data_val in dataset_astro_val:
                ...
                #only forward pass
                with amp.autocast():
                    output_val = sup_head(encoder(image_val))
        ...
        if acc_val.item() > best_acc_till_now:
            ...
            best_epoch = epoch
            save_model(encoder, sup_head, epoch, optimizer, scheduler)
