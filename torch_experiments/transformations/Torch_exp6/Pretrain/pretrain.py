...
FLAGS = flags.FLAGS
### Define Flags ###
flags.DEFINE_integer('EfficientNetv2_size', 0, 'Size of the EfficientNetv2 model. It could be only 0,1,2. 0 for small, 1 for medium and 2 for large')
flags.DEFINE_float('lr', 0.45, 'Learning Rate')
flags.DEFINE_float('weight_decay', 1e-4, 'Weight decay to use')
flags.DEFINE_float('momentum', 0.9, 'Momentum for optimizer')
flags.DEFINE_float('temperature', 0.15, 'Temperature parameter for NTXent Loss')
...
def main(argv):
    ...
    if FLAGS.EfficientNetv2_size == 0:
        encoder = timm.create_model('tf_efficientnetv2_s', pretrained=False, num_classes=0).cuda()
    elif FLAGS.EfficientNetv2_size == 1:
        encoder = timm.create_model('tf_efficientnetv2_m', pretrained=False, num_classes=0).cuda()
    elif FLAGS.EfficientNetv2_size == 2:
        encoder = timm.create_model('tf_efficientnetv2_l', pretrained=False, num_classes=0).cuda()
    else:
        raise ValueError("Not Implemented EfficientNetv2 :c")

    #Model to device
    proj_head = md.Projection_Head(1280).cuda() #1280 represents the output shape from all efficientnets implemented here
    ...
    dataset_astro = DataLoader(astro_ds,batch_size=FLAGS.batch_size, shuffle=True,num_workers=12)
