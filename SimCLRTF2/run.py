...
def main(argv):
    ...
    #deleted use with TPUs, only use for GPUs
    ...
    if FLAGS.mode == 'eval':
        ...
        with strategy.scope():
            ...
            if FLAGS.dataset == 'astro':
                print('You are using a custom dataset: astro')
                ds = data_astro.build_distributed_dataset(FLAGS.train_batch_size, True, strategy)
            else:
