...
import data_util_fine as data_util
import lars_optimizer
import resnet_fine as resnet
...
def build_optimizer(learning_rate):
    ...
    if FLAGS.optimizer == 'momentum':
        ...
        opt = lars_optimizer.LARSOptimizer(learning_rate, momentum=FLAGS.momentum, weight_decay=FLAGS.weight_decay,
            exclude_from_weight_decay=['batch_normalization', 'bias', 'head_supervised'])
        #opt = mixed_precision.LossScaleOptimizer(opt)
        return opt

    elif FLAGS.optimizer == 'novograd':
        opt = tfa.optimizers.NovoGrad(learning_rate=learning_rate, beta_1=0.95, beta_2=0.98, 
                                        epsilon=1e-08, weight_decay=FLAGS.weight_decay, amsgrad=False)
        #opt = mixed_precision.LossScaleOptimizer(opt)
        return opt
...
class Model(tf.keras.models.Model):
    ...
    def __call__(self, inputs, training):
        ...
        #add heads
        _, supervised_head_inputs = self._projection_head(hiddens, training)
